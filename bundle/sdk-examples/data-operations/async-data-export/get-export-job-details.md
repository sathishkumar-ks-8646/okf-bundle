---
type: SDK Example
title: SDK examples - Get Export Job Details
description: "Code samples in 9 languages for GET /restapi/v2/bulk/workspaces/{workspace-id}/exportjobs/{job-id} (getExportJobDetails)."
...
api:
  ...
  endpoint_doc: "/domains/data-operations/async-data-export/get-export-job-details.md"
  languages:
    - cURL
    - "C#"
    - Go
...
sources:
  ...
  - id: endpoint-doc
    resource: "/domains/data-operations/async-data-export/get-export-job-details.md"
    title: Endpoint reference - Get Export Job Details
generated:
...
Code samples for [Get Export Job Details](/domains/data-operations/async-data-export/get-export-job-details.md) (`GET /restapi/v2/bulk/workspaces/{workspace-id}/exportjobs/{job-id}`). Replace the placeholder client ID, client secret, refresh token, organization ID, workspace ID and view ID values with your own. The SDK client construction pattern for each language is explained in [SDK clients](/foundations/sdk-clients.md).
...
```bash
curl "https://analyticsapi.zoho.com/restapi/v2/bulk/workspaces/35130000001055707/exportjobs/35130000001056301" -H 'ZANALYTICS-ORGID: 55522777' -H 'Authorization: Zoho-oauthtoken <access_token>'
```
...
namespace ZohoAnalyticsTest
{
    class Program
    {
        ...
        public void GetExportJobDetails(IAnalyticsClient ac)
        {
            ...
            JsonElement result = bulk.GetExportJobDetails(jobId);
...
public class Test {
    ...
    public void getExportJobDetails(AnalyticsClient ac) throws Exception {
        ...
        JSONObject result = bulk.getExportJobDetails(jobId);
...
<?php
    ...
    class Test
    {
        ...
        function getExportJobDetails() {
            ...
            $response = $bulk->getExportJobDetails($this->job_id);
            print_r($response);
        }
    }
...
class sample:
    ...
    def get_export_job_details(self, ac):
        ...
        result = bulk.get_export_job_details(Config.JOBID)
        print(result)
...
- [Get Export Job Details](/domains/data-operations/async-data-export/get-export-job-details.md) - full endpoint reference.
